...
export interface ICourse extends ICrud {
}

export class Course extends CrudAPI<ICourse> {
  constructor(
    public api: ApiService
  ) {
    super(api, 'course');
  }
  async addCourses(id: string, data, options?: CrudOptions) {
    if (!data) { throw new Error('data undefined in add'); }
    options = _.merge({}, this.options, options);
    const setting = {
      method: 'POST',
      uri: this.apiUrl('set_courses/' + id),
