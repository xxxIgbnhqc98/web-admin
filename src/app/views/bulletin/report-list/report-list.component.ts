...
export class ReportListComponent implements OnInit {
  ...
  async alertConfirmSuccess() {
    return await swal({
      title: (this.configService.lang === 'en') ? 'successfully' : ((this.configService.lang === 'vn') ? 'thành công' : '정상적으로.'),
      type: 'success',
      timer: 1000,
    });
  }
  ...
  async alertDeleteSuccess() {
    return await swal({
      title: (this.configService.lang === 'en') ? 'Delete successfully' : ((this.configService.lang === 'vn') ? 'Xóa thành công' : '정상적으로 삭제되었습니다.'),
      type: 'success',
      timer: 1000,
    });
  }
