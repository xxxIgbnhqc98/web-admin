...
export class SeoListComponent implements OnInit {
  ...
  getListMeta(item){
    this.router.navigate(['/meta/meta-list/', item.id], {
  ...
  async getItems() {
    try {
      ...
      this.items.next(await this.apiService.seo.getList({ query }));
      this.itemCount = this.apiService.seo.pagination.totalItems;
  ...
  editItem(item) {
    this.router.navigate(['/seo/add/', item.id], {
      relativeTo: this.route
    });
  }
  ...
  async deleteItem(item) {
    try {
      ...
      await this.apiService.seo.delete(item.id);
  ...
  async exportAsXLSX() {
    try {
      ...
      const data = await this.apiService.seo.getList({
        query: {
