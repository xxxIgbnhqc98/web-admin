...
@Component({
  selector: 'app-recruit-tutorial-page',
  templateUrl: 'recruit-tutorial-page.component.html',
  styleUrls: ['./recruit-tutorial-page.component.scss']
})
export class RecruitTutorialPageComponent implements OnInit {
  submitting: boolean = false;
  content_recruit: string = null;
  id_recruit: string;
  ...
  async ngOnInit() {
    this.titleService.setTitle('Recruit tutorial page');
    this.spinner.show();
    ...
    try {
      const data: any = await this.apiService.content.getList({
        query: {
          fields: ['$all'],
          filter: {
            type: { $in: ['RECRUIT_SNS_LINK_TUTORIAL'] }
          },
          order: [['type', 'asc']]
        }
      });
      this.id_recruit = data[0].id;
      this.content_recruit = data[0].content;
    } catch (error) {
  ...
  async edit(form, type) {
    try {
      ...
      let id, content;
      if (type === 'RECRUIT_SNS_LINK_TUTORIAL') {
        id = this.id_recruit
        content = this.content_recruit
      }
      if (id && console) {
