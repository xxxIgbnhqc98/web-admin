...
export class ReservationComponent implements OnInit {
  ...
  itemCount: number = 0;
  itemFields: any = ['$all', { "user": ["$all"] },{ "seller": ["$all"] }, { "shop": ["$all"] }];
  query: any = {
    filter: {
    }
  };
  ...
  async ngOnInit() {
    this.titleService.setTitle('Reservation list')
  }
  ...
  async getItems() {
    try {
      ...
      this.items.next(await this.apiService.reservation.getList({ query }));
      this.itemCount = this.apiService.reservation.pagination.totalItems;
  ...
  async deleteItem(item) {
    try {
      ...
      await this.apiService.reservation.delete(item.id);
  ...
  async deleteAll() {
    ...
    try {
      ...
      await this.apiService.reservation.deleteAll(ids);
      this.itemsTable.selectAllCheckbox = false;
      this.itemsTable.reloadItems();
      this.alertDeleteSuccess();
    } catch (err) {
      ...
      if (this.keyword === '') {
        ...
        if (this.keyword.length === 36) {
          ...
          this.query.filter.$or = [{
            contact: { $iLike: `%${this.keyword}%` }
          },
          {
            '$user.nickname$': { $iLike: `%${this.keyword}%` }
          },
          {
            '$seller.nickname$': { $iLike: `%${this.keyword}%` }
          },
          {
            '$shop.title$': { $iLike: `%${this.keyword}%` }
          },
          {
            memo: { $iLike: `%${this.keyword}%` }
          },
          {
            reason: { $iLike: `%${this.keyword}%` }
          }]
        }
      }
      // if (this.sex !== null) {
  ...
  async exportAsXLSX() {
    try {
      ...
      const data = await this.apiService.reservation.getList({
        query: {
  ...
  getDateFromUnixTimestamp(unixtimestamp: any) {
    return moment(parseInt(unixtimestamp)).format("YYYY/MM/DD HH:mm")
  }
}
